...
class DialogController {
    ...
    public createMessage(username: String, message: String, roomID: any) {
        const self = this
        return new Promise((resolve, reject) => {
            DialogModel.updateOne({ _id: roomID },
                { $push: { messages: { message, username, timestamp: Date.now() } } }, function (err: any, res: any) {
                    if (err) throw err
    ...
    public getMessages(username: String, query: any) {
        return new Promise((resolve, reject) => {
            UserModel.findOne({ username }, { _id: 0 }, function (err: any, doc: any) {
                ...
                DialogModel.aggregate([
                    { $match: { _id: { $in: doc.messages } } },
                    { $unset: ['messages', '__v'] }
